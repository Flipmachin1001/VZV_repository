...
Uint16 MMCSD_singleBlkReadGraphic( Uint32 cardMemAddr, Uint16* dest, Uint32 blklength, Uint32 endian )
{
    Uint16 i;
    Uint16 status = 0;
...
//            return E_INVALID_INPUT;
   ...
   if ( MMCSD_setDataSize( 420, blklength ) )//450
        return E_FAIL;
    ...
    if ( MMCSD_sendCmd( 0x12000 | MMCSD_READ_MULTIPLE_BLOCK, cardMemAddr, 0, MMCSD_STAT0_RSPDNE ) )
        return E_FAIL;


    if ( MMCSD_readNWordsGraphic( dest, blklength, cardMemAddr ) )
        return E_FAIL;
...
Uint16 MMCSD_singleBlkRead( Uint32 cardMemAddr, Uint16* dest, Uint32 blklength, Uint32 endian )
{
    Uint16 i;
    Uint16 status = 0;
...
//            return E_INVALID_INPUT;
   ...
   if ( MMCSD_setDataSize( 2, blklength ) )
        return E_FAIL;
    ...
    if ( MMCSD_readNWords( dest, blklength, cardMemAddr ) )
        return E_FAIL;
...
Uint16 MMCSD_singleBlkWrite( Uint32 cardMemAddr, Uint16 *src, Uint32 blklength, Uint32 endian )
{
    Uint16 status;
...
//            return E_INVALID_INPUT;
    ...
    status = MMCSD_setDataSize( 2, blklength ); /* num_blks = 1, num_of_bytes = blklength */
    ...
    if ( status == 0 )
    {
        ...
        status = MMCSD_writeNWords( src, blklength, cardMemAddr );
                        ...
                        MMCSD_ConfigData* config, Uint16 fifoThrlevel )
{
    ...
    MMCSD_MMCCLK = 0x0100;                          // Enable the Clock and set the divide-down value//0x0102;
